...
void main() {
    ELEMENT_TYPE tet = getElement(inData[0].element);
            ...
            while(p.z<pmax.z) {
                   ...
                   lam.x<1 && lam.y<1 && lam.z <1 && lam.w <1 )
                {
                    ...
                    if(counter>=filter_first) {
                      pos = p.xyz;
                      val = EvaluateVec(FUNCTION, inData[0].element, lam.xyz);
                      EmitVertex();
                      EndPrimitive();
                    }
                }
...
void main() {
    ELEMENT_TYPE tet = getElement(inData[0].element);
    float dmin = dot(vec4(tet.pos[0],1), clipping_planes[0]);
    float dmax = dot(vec4(tet.pos[0],1), clipping_planes[0]);
    for (int i=1; i<4; i++) {
        dmin = min(dmin, dot(vec4(tet.pos[i],1), clipping_planes[0]));
        dmax = max(dmax, dot(vec4(tet.pos[i],1), clipping_planes[0]));
    }
    ...
    base[2] = normalize(clipping_planes[0].xyz);
    vec3 n = abs(clipping_planes[0].xyz);
