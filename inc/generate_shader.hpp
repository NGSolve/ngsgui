...
//         constexpr ELEMENT_TYPE type = ET_TRIG;
//         TIP<2,CCode> ip(x,y);
        constexpr ELEMENT_TYPE type = ET_TET;
        TIP<3,CCode> ip(x,y,z);

        MyFEL<type, L2HighOrderFE<type>> fel(order);
        //   MyFEL<type, L2HighOrderFEFO<type,order>> fel;
        ...
        fel.MyCalcShape (ip, SBLambda([&] (int i, auto c) {
                                      ss << "result += texelFetch( tex, " + ToString(i) + ").r * " + c.s << ";" << endl;
                                      }));
        ...
        f << "}" << endl;
        return shaders::fragment::header
               + f.str()
               + shaders::fragment::main;
    }
}
